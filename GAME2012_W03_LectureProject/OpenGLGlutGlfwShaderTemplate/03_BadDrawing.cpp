...
GLshort cube_indices[] = {
	...
	// Mid-Right.
	5, 4, 8, 9,
	// Mid-Middle. Drawn CW so it will be culled.
	5, 6, 10, 9,
	// Mid-Left.
	11, 7, 6, 10,
	// Bottom-Left.
...
void display(void)
{
	...
	glBindVertexArray(vao);
		...
		transformObject(scale, XY_AXIS, 0.0f, glm::vec3(0.0f, 0.0f, 0.0f));

		// Ordering the GPU to start the pipeline
		glDrawElements(GL_QUADS, 36, GL_UNSIGNED_SHORT, 0); 
		// glDrawElements(GL_LINE_STRIP, 36, GL_UNSIGNED_SHORT, 0); // 36 is number of indices.
		// glDrawElements(GL_LINE_LOOP, 36, GL_UNSIGNED_SHORT, 0); 
		/* GL_LINE_LOOP is for entire shape. We can restart the primitive though with 
		glEnable(GL_PRIMITIVE_RESTART_FIXED_INDEX) I will demonstrate this in an example later. */
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("03_BadDrawing");
